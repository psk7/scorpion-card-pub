...
#pragma pack(push, 1)
    ...
    typedef USB_STR_DESCRIPTOR *PUSB_STR_DESCRIPTOR;
    ...
    typedef struct {
        PBElement TX_Address;
        PBElement TX_Count;
        PBElement RX_Address;
        PBElement RX_Count;
    } EPBuf;
...
//void HandleStatus(void);
    ...
    void SetEPTable(uint8_t EP, uint32_t TXAddress, uint32_t TXCount, uint32_t RXAddress, uint32_t RXCount);

    void LogCheckForTransmit();

    void Transmit(uint16_t *Data, uint16_t Length);

    void LineStateHandler(WByte LineState);

    void CompleteReading(uint8_t Endpoint);

/*__weak*/ bool DataReceivedHandler(uint8_t Endpoint, uint16_t *Data, uint16_t Length);

/*__weak*/ void DataTransmitedHandler(uint8_t EPn, EPData EpData);

#endif

    uint8_t *GetDeviceDescriptor(int *Length);

    uint8_t *GetConfigurationDescriptor(int *Length);

    uint8_t *GetString(int Index, int *Length);
}