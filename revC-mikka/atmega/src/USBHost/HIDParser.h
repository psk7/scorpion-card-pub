...
#define __HIDPARSER_H__
	...
	/* Macros: */
			 ...
			 */
			#define HID_STATETABLE_STACK_DEPTH    4
		#endif
	...
	/* Public Interface - May be used in end-application: */
		...
		/* Type Defines: */
			...
			typedef struct
			{
				uint16_t Minimum; /**< Minimum value for the attribute. */
				uint16_t Maximum; /**< Maximum value for the attribute. */
			} HID_MinMax_t;
			...
			typedef struct
			{
				...
				uint16_t ReportSizeBits[1]; /**< Total number of bits in each report type for the given Report ID,
    ...
    class HID_ReportItem {
            public:
                HID_ReportItem(uint8_t bit, uint8_t id) : bit_offset(bit), bit_size(1), report_id(id), log_min(0), log_max(1) {};
                HID_ReportItem(uint8_t bit) : bit_offset(bit), bit_size(1), report_id(1), log_min(0), log_max(1) {};
                HID_ReportItem() : bit_offset(0), bit_size(0), report_id(0), log_min(0), log_max(0) {};

                HID_ReportItem(const HID_ReportItem_t & From);

                uint16_t bit_offset;
                uint8_t  bit_size;
                uint8_t  report_id;
                uint8_t  log_min, log_max;
                uint8_t  k = 0;
            };
    ...
    class HIDParser {
        ...
        USBH::task<uint8_t> ParseDescriptor(USBH::StreamReader<> &Source, uint16_t ReportSize,
                HID_ReportInfo_t *const ParserData);

        uint32_t GetItemInfo(const uint8_t* ReportData, HID_ReportItem const &ReportItem, bool SignExtend = true);

        virtual void OnItem(const HID_ReportItem_t &CurrentItem) = 0;
    };
