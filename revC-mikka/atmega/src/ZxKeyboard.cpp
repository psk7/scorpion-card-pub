...
void ZxKeyboard::MapJoystickAndSend(KeysList &List) {
    List.clear();

    KeyboardMapper mapper(List, zxLayoutReference);

    mapper.MapKeyboard(IsLeftShiftPressed || IsRightShiftPressed, RusLayout, Keys);
}
...
struct JoystickMappingInfo::ZxJoystickMapping ZxKeyboard::GetZxJoystickMapping() const {
    struct JoystickMappingInfo::ZxJoystickMapping v{};

    ConfigStorage.ReadZxJoystickMappings(JoyMapNum, v);

    return v;
}

struct JoystickMappingInfo::Mapping::Bindings ZxKeyboard::GetJoystickMapping() const {
    struct JoystickMappingInfo::Mapping::Bindings v{};

    ConfigStorage.ReadJoystickMappings(JoyMapNum, v);
