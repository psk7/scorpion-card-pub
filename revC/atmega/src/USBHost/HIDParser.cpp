...
uint8_t HIDParser::ParseDescriptor(const uint8_t *ReportData, uint16_t ReportSize, HID_ReportInfo_t *const ParserData) {
            ...
            case HID_RI_FEATURE(0):
                for (uint8_t ReportItemNum = 0; ReportItemNum < CurrStateTable->ReportCount; ReportItemNum++) {
                    ...
                    ParserData->LargestReportSizeBits = MAX(ParserData->LargestReportSizeBits,
                                                            CurrReportIDInfo->ReportSizeBits[NewReportItem.ItemType]);
...
uint32_t HIDParser::GetItemInfo(const uint8_t *ReportData, HID_ReportItem const &ReportItem) {
